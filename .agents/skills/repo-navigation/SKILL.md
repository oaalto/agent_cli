...
Source under `src/main/kotlin/com/oaalto/agent/`:
- `pty/` — PTY Passthrough editor and launch path
- `acp/` — ACP client, session loop, transcript/shell UI (with `acp/auth/`, `acp/filesystem/`, `acp/mcp/`, `acp/permission/`, `acp/plan/`, `acp/terminal/`, `acp/ui/` sub-packages)
- `worktree/` — Git worktree orchestration, mode-agnostic (with `worktree/resume/`)
- `settings/` — Shared configuration and launch mode (with `settings/acpjson/`)

### This repo's domain

IntelliJ Platform plugin that runs external agent CLIs in a dedicated editor tab. Key concepts: ACP (Agent Client Protocol) JSON-RPC, Launch Mode (PTY Passthrough vs ACP Client), AgentConfigurationSelector (per-project runtime selection), Worktree (isolated session filesystem).

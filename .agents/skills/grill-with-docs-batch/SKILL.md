...
- **Glossary:** `CONTEXT.md` at repo root (domain vocabulary for Agent CLI plugin, ACP, worktrees, launch modes)
- **Decisions:** `docs/adr/` (accepted ADRs — e.g. custom ACP client, Kotlin SDK, per-project agent selection)
- **Wiki:** `docs/wiki/` for subsystem behavior and workflows
- **Planning:** `docs/features/<feature_name>/` for PRDs and numbered implementation slices

Use whatever the project already has; do not impose a layout the repo does not use.
...
If `grill-with-docs` is installed and ships format guides (`CONTEXT-FORMAT.md`, `ADR-FORMAT.md`, or similar next to that skill), follow them. Otherwise mirror the style of existing docs in the repo (`CONTEXT.md`, `docs/adr/`).

...
- **Cost import consistency** (`src/main/kotlin/com/oaalto/agent/acp/TranscriptModel.kt`): Added explicit import for `com.agentclientprotocol.model.Cost` and replaced fully-qualified name usage with the import. made by: Olli Aalto. made with: Cursor. model: kimi-k2.5
- **Currency normalization** (`src/main/kotlin/com/oaalto/agent/acp/TranscriptModel.kt`): Currency codes are now normalized to uppercase when accumulating costs, ensuring consistent display across `TranscriptFooter` and `TranscriptModel`. made by: Olli Aalto. made with: Cursor. model: kimi-k2.5

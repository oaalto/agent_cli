...
## Agent CLI repository (this project)

- **Layout:** Kotlin IntelliJ plugin under `src/main/kotlin/com/oaalto/agent/` — subpackages `acp` (ACP client/editor/transcript), `worktree`, `settings`, `pty`.
- **Current graph mode:** AST-only (`graphify update . --no-cluster`). Code call/import edges are `EXTRACTED`; markdown/wiki docs are not semantically indexed until an extraction backend is configured (`graphify.env.example`).
- **Refresh:** `graphify update .` from repo root after Kotlin changes. No pre-commit hook yet.
- **Ignore:** `.graphifyignore` excludes `build/`, `.gradle/`, `.idea/`, `.intellijPlatform/`, `.pi/`, scratch dirs.
- **Domain docs:** `CONTEXT.md`, `docs/wiki/`, `docs/prds/` — use wiki for narrative; graphify for cross-file Kotlin topology.
- **Queries:** Prefer `graphify query` / `graphify path` starting from `AcpAgentEditor`, `AcpSessionController`, or `TranscriptHtmlAppender` for transcript/ACP work.
